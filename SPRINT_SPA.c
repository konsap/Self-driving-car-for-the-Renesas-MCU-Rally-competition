...
#define lineChange 55       //Max speed for safe line changing

#define STRICT 2000.0       //Need change
#define NORMAL 40.0
#define LOOSE 20.0
#define ULTRALOOSE 7.0
      ...
      switch (state) {
        ...
        case FOLLOW:
          if ((speed[0] > 65) || (speed[1] > 65) || (speed[2] > 65) || (speed[3] > 65)){
            ...
            motor(DEAD, DEAD, DEAD, DEAD);                                                  //motor(back right, back left, front right, front left). (-100 to +100)
          }
          else{
            handle(-output_pid);
            motor(ULTRAFAST, ULTRAFAST, ULTRAFAST, ULTRAFAST); 
          }
          if (fabs(line) > 0.04){                                                           //Turn detected
            printf("TURNING\n");
            if ((speed[0] > 55) || (speed[1] > 55) || (speed[2] > 55) || (speed[3] > 55)){
              //printf("BREAKING\n");
          ...
          if (angles[1] > 0.02){        	                                                  //upward pitch rotation was detected
            state = RAMP_UP;
            printf("RAMP UP\n");
          }
          ...
          else if (totalWhiteSensors == 8){                                                 //Fisrt full strip was detected
            state = CORNER;
            cornerState = stripDetected;
          ...
          switch (lineChangeState){
            case halfStripDetected:
              if (totalWhiteSensors == 8){
                ...
                if ((speed[0] > 70) || (speed[1] > 70) || (speed[2] > 70) || (speed[3] > 70)){
                  motor(BRAKE, BRAKE, BRAKE, BRAKE);
                  handle(STRICT * line);
                }
                else{
            ...
            case lineChangeHappening:
              switch (determinedChange){
                case leftLineChange:
                  ...
                  if ((totalWhiteSensors == 0) && (fabs(fabs(initDirection) - fabs(angles[2])) < 0.29)){
                    printf("1) turning ");
                    motor(lineChange, lineChange, lineChange, lineChange);
                    handle(30);
                  }
                  else if ((totalWhiteSensors == 0) && (fabs(fabs(initDirection) - fabs(angles[2])) >= 0.29)){
                    printf("2) straightening ");
                    motor(lineChange, lineChange, lineChange, lineChange);
                    handle(-30);
                  }
                  else if ((totalWhiteSensors >= 1) && (fabs(fabs(initDirection) - fabs(angles[2])) > 0.40)){
                    printf("3a) prepairing follow ");
                    motor(lineChange, lineChange, lineChange, lineChange);
                    handle(-30);
                  }
                  else if ((totalWhiteSensors >= 1) && (fabs(fabs(initDirection) - fabs(angles[2])) > 0.15)){
